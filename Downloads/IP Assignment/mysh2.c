...
char **parse_line(char *line)
{
	...
	{	
		...
		while(token != NULL){
			...
			// if the args is full we need to re allocate its size
			...
			// in the next call to strtok(), the first parameter needs to be NULL so that strtok starts splitting the string from the next token's starting position
			token = strtok(NULL,DELIMITER);
		}
		// execute function expects a NULL in the end
		args[position]= NULL;
	}
